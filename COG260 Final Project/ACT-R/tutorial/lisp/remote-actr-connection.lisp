...
;;; 2024.05.01 Dan
;;;             : * Replace acquire-lock calls with with-lock-held to be safe
;;;             :   for use with SBCL.
;;;             : * Need to shadow reset for use with SBCL.
;;; 2024.06.14 Dan
;;;             : * Reordered some of the code so that it doesn't result in
;;;             :   warnings in SBCL.
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
#+(and (not :actr-environment) (or :allegro :ccl :sbcl)) (defun cl-user::print-error-message (stream error colon-p atsign-p &optional (width 0) padchar commachar)
         (declare (ignore colon-p atsign-p width padchar commachar))
...
;;; The set-logical.lisp file uses this and SBCL complains
;;; when using the pre-built application.

#+:actr-environment (defun compile-and-load (pathname &optional force)
                      (cl-user::compile-and-load pathname force))
