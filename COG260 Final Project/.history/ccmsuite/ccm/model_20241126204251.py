...
class Model:
    ...
    def __convert(self, parent=None, name=None):
        ...
        if self._auto_run_start:
            start_method = getattr(self, "start", None)
            print(
                f"Found start method: {start_method}, callable: {callable(start_method)}"
            )
            if start_method and callable(start_method):
                start_method(self)  # Pass `self` explicitly to ensure proper binding
    ...
    def __setattr__(self, key, value):
        print(f"Setting attribute '{key}' to '{value}' in instance {self}")
        if (
            key == "parent"
            and value is None
    ...
    def start(self):
        print(f"Starting model: {self.name}")
        if hasattr(self, "log") and self.log:
            print(f"Logging enabled for {self.name}.")
        else:
            print(f"No logging for {self.name}.")

    def run(self, limit=None, func=None):
        print(f"Running model: {self.name} with limit={limit}")
