...
class Model(object):
    ...
    def __convert(self, parent=None, name=None):
        ...
        # Only call start on self if it's the top-level model
        if self._auto_run_start and parent is None:
    ...
    def __setattr__(self, key, value):
        # Commented out debug print statement to clean up output
        # print("Setting attribute '{}' to '{}' in instance {}".format(key, value, self))
        if (
            key == "parent"
            and value is None
    ...
    def run(self, limit=None, func=None):
        # Commented out debug print statement to clean up output
        # print("Running model: {} with limit={}".format(self.name, limit))
    ...
    # Removed the start method to avoid interfering with subclasses
    # def start(self):
    #     pass
