...
class MemoryEnvironment(ccm.Model):
    def start(self):
        print(f"Starting MemoryEnvironment: {self.name}")
        print(f"Type of self: {type(self)}")
        if not callable(super().start):
            print("Error: Parent class 'start' method is not callable!")
        else:
            super().start()  # Explicitly call the parent class's `start` method

    def run(self, *args, **kwargs):
        print(f"Running MemoryEnvironment: {self.name}")
...
def run_simulation(condition):
    ...
    print("Forcing conversion of environment...")
    environment._ensure_converted()
    ...
    if hasattr(environment, "start") and callable(getattr(environment, "start")):
        environment.start()
    else:
        print("Error: 'start' is not callable!")

    print("Starting agent...")
    if hasattr(agent, "start") and callable(getattr(agent, "start")):
        agent.start()
    else:
        print("Error: 'agent.start' is not callable!")

    print(f"Calling run on environment: {environment}")
    if hasattr(environment, "run") and callable(getattr(environment, "run")):
        environment.run()
    else:
        print("Error: 'run' is not callable!")
